...
# Function to switch to adding events
def switch_to_mero():

    attraction_name_label.forget()
    attraction_name_entry.forget()
    attraction_description_label.forget()
    attraction_description_entry.forget()
    image_path_label.forget()
    image_path_entry.forget()
    adress_label.forget()
    adress_entry.forget()

# Function to switch to adding attractions
def switch_to_attraction():
...
add_date_button = tk.Button(root, text="Add Date", command=create_attraction_frame)
add_date_button.pack()
# Rest of your code remains the same

# Menu creation for adding an event or an attraction
menu_frame = tk.Frame(root)
menu_frame.pack(padx=10, pady=10)

attraction_button = tk.Button(menu_frame, text="Добавить достопримечательность", command=switch_to_attraction)
attraction_button.pack(side=tk.LEFT, padx=5)

mero_button = tk.Button(menu_frame, text="Добавить мероприятие", command=switch_to_mero)
mero_button.pack(side=tk.RIGHT, padx=5)
#
# mero_label = tk.Label(root, text="Дата:")
# mero_entry = tk.Entry(root)
